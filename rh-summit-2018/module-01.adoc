...
Starting from the current Kafka cluster with 3 brokers, we want to add a new one.
...
It opens the default editor that we can use in order to change the value of the `kafka-nodes` field from 3 to 4.
After saving the file, the Cluster Controller detects the update and starts a new broker Pod; it's just a simple scale-up operation.
